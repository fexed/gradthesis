...
import time
import copy
import ascertain_ewc_train as EWC


# Log directory
def get_run_logdir(label=""):
    import time
    run_id = time.strftime("ewc_ASCERTAIN_%Y%m%d_%H%M%S")
...
for S in [("S0", "S1"), ("S2", "S3"), ("S4", "S5"), ("S6", "S7"), ("S8", "S9"), ("S10", "S11"), ("S12", "S13"), ("S14", "S15")]:
    ...
    start = time.time()
    if (F is None and theta_A is None):
        F, theta_A, nepochs = EWC.train_ewc(model, Xtr, ytr, Xvl, yvl, epochs=100, callbacks=[es, tb], batch_size=256, use_multiprocessing=True, workers=8)
    else:
        F, theta_A, nepochs = EWC.train_ewc(model, Xtr, ytr, Xvl, yvl, epochs=100, callbacks=[es, tb], F=F, theta_A=theta_A)
