...
def get_run_logdir(label=""):
    import time
    run_id = time.strftime("continual_episodicpred_wesad_%Y%m%d_%H%M%S")
...
model.add(tf.keras.layers.LSTM(30, input_shape=(100, 14)))
...
for S in [("S2", "S3"), ("S4", "S5"), ("S6", "S7"), ("S8", "S9"), ("S10", "S11"), ("S13", "S14"), ("S15", "S16")]:
    ...
    for i in idx_1:
        X_1.append(Xtr[i])
        y_1.append(model.predict(Xtr[i:i+1]).reshape(4))
    random.shuffle(X_1)
    X_1 = X_1[0:m]
    y_1 = y_1[0:m]
    ...
    for i in idx_2:
        X_2.append(Xtr[i])
        y_2.append(model.predict(Xtr[i:i+1]).reshape(4))
    random.shuffle(X_2)
    X_2 = X_2[0:m]
    y_2 = y_2[0:m]
    ...
    for i in idx_3:
        X_3.append(Xtr[i])
        y_3.append(model.predict(Xtr[i:i+1]).reshape(4))
    random.shuffle(X_3)
    X_3 = X_3[0:m]
    y_3 = y_3[0:m]
    ...
    for i in idx_4:
        X_4.append(Xtr[i])
        y_4.append(model.predict(Xtr[i:i+1]).reshape(4))
    random.shuffle(X_4)
    X_4 = X_4[0:m]
    y_4 = y_4[0:m]
...
plt.plot(scores)
plt.savefig("accuracy_continual_episodicpred.png")
