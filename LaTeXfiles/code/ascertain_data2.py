...
for i in range(37, 57):
    if(i not in excluded_subjects):
        ...
        clips_ECG = {}
        for j in range(1, 37):
            clips_ECG[('n_' + str(j))] = scipy.io.loadmat('ASCERTAIN/ECGData/Movie_P' + str(i) + '/ECG_Clip' + str(j))['Data_ECG'][:,1:]
        clips_EEG = {}
        for j in range(1, 37):
            clips_EEG[('n_' + str(j))] = np.transpose(scipy.io.loadmat('ASCERTAIN/EEGData/Movie_P' + str(i) + '/EEG_Clip' + str(j))['ThisEEG'][:,1:])
        clips_GSR = {}
        for j in range(1, 37):
            clips_GSR[('n_' + str(j))] = scipy.io.loadmat('ASCERTAIN/GSRData/Movie_P' + str(i) + '/GSR_Clip' + str(j))['Data_GSR'][:,1:]

        arousal = scipy.io.loadmat('ASCERTAIN/Dt_SelfReports.mat')['Ratings'][0][i]
        valence = scipy.io.loadmat('ASCERTAIN/Dt_SelfReports.mat')['Ratings'][1][i]
        print(" loaded", end = "")
        ...
        for clip in clips.keys():
           if(((Y[j] == 0) & (count_0 < 84)) or ((Y[j] == 1) & (count_1 < 84)) or ((Y[j] == 2) & (count_2 < 84)) or ((Y[j] == 3) & (count_3 < 84))):
               ...
               for k in range(0, 10):
                   SubsequencesX.append(clips[clip][length-(160*(k+1)):length-(160*k),:])
                   if (Y[j] == 0):
                        count_0 += 1
        ...
        # Salvataggio
        with open("ASCERTAIN/splitted/XS" + str(n) + ".pkl", 'wb') as handle:
            pickle.dump(Xs, handle, protocol=pickle.HIGHEST_PROTOCOL)
        with open("ASCERTAIN/splitted/yS" + str(n) + ".pkl", 'wb') as handle:
...
for S in ["S0", "S1", "S2", "S3", "S4", "S5", "S6", "S7", "S8", "S9", "S10", "S11", "S12", "S13", "S14", "S15", "S16"]:
    Xs = pickle.load(open("ASCERTAIN/splitted/X" + S + ".pkl", 'rb'), encoding='latin1')
    ys = pickle.load(open("ASCERTAIN/splitted/y" + S + ".pkl", 'rb'), encoding='latin1')
...
with open("ASCERTAIN/splitted/Xts.pkl", 'wb') as handle:
    pickle.dump(Xts, handle, protocol=pickle.HIGHEST_PROTOCOL)
with open("ASCERTAIN/splitted/yts.pkl", 'wb') as handle:
...
for S in ["S0", "S1", "S2", "S3", "S4", "S5", "S6", "S7", "S8", "S9", "S10", "S11", "S12", "S13", "S14", "S15", "S16"]:
    ...
    with open("ASCERTAIN/splitted/X" + S + ".pkl", 'wb') as handle:
        pickle.dump(Xs, handle, protocol=pickle.HIGHEST_PROTOCOL)
    with open("ASCERTAIN/splitted/y" + S + ".pkl", 'wb') as handle:
        pickle.dump(ys, handle, protocol=pickle.HIGHEST_PROTOCOL)
